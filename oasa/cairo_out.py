...
class cairo_out:
    ...
    def mols_to_cairo(self, mols, filename, format="png"):
        ...
        self.context.translate(
            round(-x1 * self.scaling + self.scaling * self.margin),
    ...
    def _draw_edge(self, e):
        ...
        def draw_plain_or_colored_wedge(_start, _end):
            x1, y1 = _start
            x2, y2 = _end
            x, y, x0, y0 = geometry.find_parallel(
                x1, y1, x2, y2, self.wedge_width / 2.0
            )
            xa, ya, xb, yb = geometry.find_parallel(
                x1, y1, x2, y2, self.line_width / 2.0
            )
            # no coloring now
            if not has_shown_vertex or not self.color_bonds:
                self._create_cairo_path(
                    [
                        (xa, ya),
                        (x0, y0),
                        (2 * x2 - x0, 2 * y2 - y0),
                ...
                self._create_cairo_path(
                    [(xa, ya), (xm1, ym1), (xm2, ym2), (2 * x1 - xa, 2 * y1 - ya)],
                    closed=True,
                )
                self.context.fill()
                self.context.set_source_rgb(*color2)
                self._create_cairo_path(
                    [(xm1, ym1), (x0, y0), (2 * x2 - x0, 2 * y2 - y0), (xm2, ym2)],
                    closed=True,
                )
                self.context.fill()
        ...
        if coords:
            ...
            if e.order == 2:
                ...
                for (
                    ...
                    self.molecule.get_smallest_independent_cycles_dangerous_and_cached()
                ):
                    double_bonds = len(
                        [
                            b
                            for b in self.molecule.vertex_subgraph_to_edge_subgraph(
                                ring
                            )
                            if b.order == 2
                        ]
                    ...
                    if v1 in ring and v2 in ring:
                        in_ring = True
                        side += double_bonds * reduce(
                            operator.add,
                            [
                                geometry.on_which_side_is_point(start + end, (a.x, a.y))
                                for a in ring
                                if a != v1 and a != v2
                            ],
                        )
                ...
                if side:
                    ...
                    if v2 not in self._vertex_to_bbox:
                        x2, y2 = geometry.elongate_line(
                            x1, y1, x2, y2, -self.bond_second_line_shortening * length
                        )
                    if v1 not in self._vertex_to_bbox:
                        x1, y1 = geometry.elongate_line(
                            x2, y2, x1, y1, -self.bond_second_line_shortening * length
                        )
                ...
                for i in (1, -1):
                    x1, y1, x2, y2 = geometry.find_parallel(
                        start[0], start[1], end[0], end[1], i * self.bond_width * 0.7
                    )
                    draw_plain_or_colored_line((x1, y1), (x2, y2), second=True)
    ...
    def _where_to_draw_from_and_to(self, b):
        ...
        if bbox1:
            x1, y1 = geometry.intersection_of_line_and_rect(
                (x1, y1, x2, y2), bbox1, round_edges=0
            )
        if bbox2:
            x2, y2 = geometry.intersection_of_line_and_rect(
                (x1, y1, x2, y2), bbox2, round_edges=0
            )
    ...
    def _find_place_around_atom(self, atom):
        ...
        angles = [
            geometry.clockwise_angle_from_east(x1 - x, y1 - y) for x1, y1 in coords
        ]
    ...
    def _draw_vertex(self, v):
        ...
        if show_symbol:
            ...
            bbox = self._draw_text(
                (x, y), text, center_letter=center_letter, color=color
            )
            self._vertex_to_bbox[v] = (
                (x, y),
                geometry.expand_rectangle(bbox, self.space_around_atom),
            )
            ...
            if charge:
                ...
                (
                    ...
                    x_advance,
                    y_advance,
                ) = self.context.text_extents(charge)
                ...
                x1, y1 = geometry.intersection_of_line_and_rect(
                    line, bbox, round_edges=0
                )
                x2, y2 = geometry.intersection_of_line_and_rect(
                    line, charge_bbox, round_edges=0
                )
                ...
                self.context.move_to(
                    round(x - xbearing - 0.5 * width), round(y + 0.5 * height)
                )
    ...
    def _draw_line(
        self, start, end, line_width=1, capstyle=cairo.LINE_CAP_BUTT, color=(0, 0, 0)
    ):
        self.context.set_source_rgb(*color)
        # cap style
    ...
    def _draw_text(
        ...
        if center_letter == "first":
            ...
            (
                ...
                x_advance,
                y_advance,
            ) = self.context.text_extents(chunks[0].text[0])
            x -= 0.5 * x_advance
            y += 0.5 * height
            ...
            for i, chunk in enumerate(chunks):
                if "sup" in chunk.attrs or "sub" in chunk.attrs:
                    self.context.set_font_size(
                        int(font_size * self.subscript_size_ratio)
                    )
                else:
                    self.context.set_font_size(font_size)
                (
                    ...
                    x_advance,
                    y_advance,
                ) = self.context.text_extents(chunk.text)
                _dx += x_advance
            # last letter
            (
                xbearing,
                ...
                x_advance,
                y_advance,
            ) = self.context.text_extents(chunk.text[-1])
            x -= _dx - 0.5 * x_advance
            y += 0.5 * height
