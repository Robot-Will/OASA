...
class atom(chem_vertex):
    ...
    def _get_occupied_valency(self):
        ...
        if self.charge:
            ...
            if abs(self.charge) > 1:
                ...
                (self.symbol in PT.accept_cation)
                and (self.charge == 1)
                and (self.valency <= PT.accept_cation[self.symbol])
            ):
                # elements that can accept cations to increase their valency (NH4+)
                charge = -1
            elif (
                (self.symbol in PT.accept_anion)
                and (self.charge == -1)
                and (self.valency <= PT.accept_anion[self.symbol])
            ):
    ...
    electronegativity = property(
        _get_electronegativity, None, None, "atoms electronegativity"
    )
    ...
    def _get_oxidation_number(self):
        ...
        for e, n in self.get_neighbor_edge_pairs():
            if isinstance(n, atom) and n.symbol != self.symbol:
                en += e.order * (
                    n.electronegativity > self.electronegativity and 1 or -1
                )
        hen = PT.periodic_table["H"]["en"]
        en += self.free_valency * (hen > self.electronegativity and 1 or -1)
        return en

    oxidation_number = property(
        _get_oxidation_number, None, None, "atoms oxidation number"
    )

    # electron pairs
    def _get_electron_pairs(self):
        return (
            PT.periodic_table[self.symbol]["els"]
            - sum(b.order for b in self.neighbor_edges)
            - self.charge
            - self.free_valency
            - self.multiplicity
            + 1
    ...
    electron_pairs = property(
        _get_electron_pairs, None, None, "get number of atoms electron pairs"
    )
        ...
        while not is_uniquely_sorted(cips, cip_sorting_function):
            for cip in cips:
                try:
                    ...
                    cip[0].append(
                        None
                    )  # None was yielded and it has no symbol_number :)
            # to test if we have already reached the end
            for i in range(len(cips) - 1):
                if cip_sorting_function(cips[i], cips[i + 1]) == 0 and cips[i][0][
                    -3:-1
                ] == [None, None]:
                    return False
