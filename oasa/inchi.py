...
class inchi(plugin):
    ...
    stereo_remap = {
        "+": cis_trans_stereochemistry.OPPOSITE_SIDE,
    ...
    def _read_inchi(self, text):
        ...
        while repeat and not self._no_possibility_to_improve:
            ...
            if not fvs and not [
                _f for _f in [not v.order for v in self.structure.edges] if _f
            ]:
                repeat = False
            else:
                if len(fvs) == 1:
    ...
    def read_sum_layer(self):
        ...
        processed_hs = (
            0  # for diborane and similar compounds we must process some Hs here
        )
        j = 0
        ...
        for c in chunks:
            if c == "(":
                ...
                try:
                    ...
                    raise ValueError(
                        "unexpected character %s in the connectivity layer" % c
                    )
                # atom
                if last_atom:
        ...
        if charge < 0:
            ...
            while charge and change:
                ...
                for v in self.structure.vertices:
                    if (
                        v.free_valency
                        and sum(n.free_valency for n in v.neighbors) < v.free_valency
                    ):
        ...
        if free_valencies % 2:
            ...
            while charge and change:
                ...
                for v in self.structure.vertices:
                    if (
                        v.symbol != "C"
                        and not v.free_valency
                        and [_f for _f in [n.free_valency for n in v.neighbors] if _f]
                    ):
                        v.charge = charge
                        charge = 0
                        change = True
    ...
    def read_p_layer(self):
        ...
        if self._protonation_dealt_with_already:
            # p was solved above
            dp = self._protonation_dealt_with_already - sum(
                v.charge for v in self.structure.vertices
            )
            if dp > 0:
    ...
    def read_double_bond_stereo_layer(self):
        ...
        for a1, a2, sign in re.findall(r"(\d+)-(\d+)([-+?u])", layer):
            ...
            st = cis_trans_stereochemistry(
                references=[neigh1, atom1, atom2, neigh2], value=self.stereo_remap[sign]
            )
