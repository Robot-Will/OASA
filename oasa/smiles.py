...
class smiles(plugin):
    ...
    def recode_oasa_to_smiles_bond(self, b):
        if b.aromatic:
            ...
            others = [
                (e, st)
                for e, st in self._stereo_bonds_to_others[b]
                if e in self._stereo_bonds_to_code
            ]
            if not others:
                code = "\\"
            else:
                ...
                if set(other.vertices) == {end1, inside1}:
                    ...
                    v1 = inside2
                    v2 = end2
                last_order = self._processed_atoms.index(
                    v2
                ) - self._processed_atoms.index(v1)
    ...
    def read_smiles(self, text, explicit_hydrogens_to_real_atoms=False):
        ...
        text = re.sub(
            r"([\\/])([0-9])",
            lambda m: (m.group(1) == "/" and "\\" or "/") + m.group(2),
            text,
        )
        # // end
        ...
        for c in chunks:
            ...
            if is_text.match(c) or c.islower() or c[0] == "[":
                ...
                if (
                    last_bond
                ):  # and not (not 'aromatic' in a.properties_ and last_bond.aromatic):
    ...
    def _check_the_chunks(self, chunks):
        ...
        for c in chunks:
            ...
            if is_text.match(c):
                if (
                    not c in PT.periodic_table and len(c) == 2
                ) or c == "Sc":  # Sc is S-c not scandium
                    a, b = c
                    ret.append(a)
                    ret.append(b)
                else:
    ...
    def _process_stereochemistry(self, mol):
        ...
        for path in paths:
            bond1 = path[0]
            end_atom1, inside_atom1 = get_end_and_inside_vertex_from_edge_path(
                bond1, path
            )
            bond2 = path[-1]
            end_atom2, inside_atom2 = get_end_and_inside_vertex_from_edge_path(
                bond2, path
            )
        ...
        for v in mol.vertices:
            ...
            if "stereo" in v.properties_:
                ...
                if len(idx) < 3:
                    ...
                    if v.explicit_hydrogens == 0:
                        ...
                        refs = (
                            [mol.vertices[i] for i in idx1]
                            + [h]
                            + [mol.vertices[i] for i in idx2]
                        )
                elif len(idx) == 4:
            ...
            if refs:
                if v.properties_["stereo"] == "@":
                    direction = (
                        stereochemistry.tetrahedral_stereochemistry.ANTICLOCKWISE
                    )
    ...
    def get_smiles(self, mol):
        if not mol.is_connected():
            raise oasa_exceptions.oasa_not_implemented_error(
                "SMILES",
                "Cannot encode disconnected compounds, such as salts etc. HINT - use molecule.get_disconnected_subgraphs() to divide the molecule to individual parts.",
            )
        # mol = molec.copy()
        self.molecule = mol
        self.ring_joins = []
        ...
        for st in mol.stereochemistry:
            if isinstance(st, stereochemistry.cis_trans_stereochemistry):
                ...
                self._stereo_bonds_to_others[e1] = self._stereo_bonds_to_others.get(
                    e1, []
                ) + [(e2, st)]
    ...
    def _get_smiles(self, mol, start_from=None):
        ...
        while not (is_line(mol) and (not start_from or start_from.get_degree() <= 1)):
            if is_pure_ring(mol):
                if start_from:
                    self.ring_joins.append(
                        mol.temporarily_disconnect_edge(start_from.neighbor_edges[0])
                    )
                else:
                    self.ring_joins.append(
                        mol.temporarily_disconnect_edge(list(mol.edges)[0])
                    )
            else:
                e, mol, branch_vertex, branch = self.disconnect_something(
                    mol, start_from=start_from
                )
                if branch_vertex:
    ...
    def _create_atom_smiles(self, v):
        ...
        if (
            (v.symbol not in self.organic_subset)
            or (v.isotope)
            or (v.charge != 0)
            or (v.valency != PT.periodic_table[v.symbol]["valency"][0])
            or (stereo)
            or (v.multiplicity != 1)
        ):
    ...
    def disconnect_something(self, mol, start_from=None):
        ...
        ring_join_vertices = set(
            reduce(operator.add, [e.vertices for e in self.ring_joins], [])
        )
        for e in mol.edges:
            d1, d2 = (x.get_degree() for x in e.get_vertices())
            if d1 > 2 or d2 > 2:
    ...
    def disconnect_something_simple(self, mol, start_from=None):
        ...
        print(
            ...
            mol.is_connected(),
            ",",
            list(map(len, mol.get_connected_components())),
            ",",
            start_from,
        )
        raise "fuck, how comes!?"
...
class smiles_converter(converter_base):
        ...
        if ">" in text:
            if text.count(">") != 2:
                raise oasa_exceptions.oasa_smiles_error(
                    "Only two '>' characters are supported in SMILES reactions. %d present"
                    % text.count(">")
                )
            parts = text.split(">")
            res = []
    ...
    def _read_string(self, text):
        ...
        sm.read_smiles(
            text,
            explicit_hydrogens_to_real_atoms=self.configuration[
                "R_EXPLICIT_HYDROGENS_TO_REAL_ATOMS"
            ],
        )
        mol = sm.structure
        if mol == None:
        ...
        for mol in mols:
            ...
            if self.configuration["R_GENERATE_COORDS"]:
                coords_generator.calculate_coords(
                    mol, bond_length=self.configuration["R_BOND_LENGTH"]
                )
        self.result = mols
        self.last_status = self.STATUS_OK
        return mols
