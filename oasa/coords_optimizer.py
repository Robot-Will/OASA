...
class coords_optimizer:
    ...
    def optimize_coords(self, mol, bond_length=1, callback=None):
        ...
        while rms_grad > (self.cutoff_rms_grad / self.bond_length) or max_grad > (
            self.cutoff_max_grad / self.bond_length
        ):
            i += 1
            if callback:
    ...
    def set_gradient(self, vertex):
        ...
        if variations:
            for var in variations:
                v1, v2 = var
                gx1, gy1, gx2, gy2 = self.get_angle_gradient(
                    deg_to_rad(120), vertex, v1, v2
                )
    ...
    def get_angle_gradient2(self, opt_angle, refv, v1, v2):
        ...
        ang = ang1 - ang2
        sign = -geometry.on_which_side_is_point(
            (refv.x, refv.y, v1.x, v1.y), (v2.x, v2.y)
        )
        ...
        gx1 = (cos(ang1 - sign * dang) - cos(ang1)) * sqrt(
            (refv.x - v1.x) ** 2 + (refv.y - v1.y) ** 2
        )  # self.bond_length #(v1.x-refv.x)
        gy1 = (sin(ang1 - sign * dang) - sin(ang1)) * sqrt(
            (refv.x - v1.x) ** 2 + (refv.y - v1.y) ** 2
        )  # self.bond_length #(v1.y-refv.y)

        gx2 = (cos(ang2 + sign * dang) - cos(ang2)) * sqrt(
            (refv.x - v2.x) ** 2 + (refv.y - v2.y) ** 2
        )  # self.bond_length #(v2.x-refv.x)
        gy2 = (sin(ang2 + sign * dang) - sin(ang2)) * sqrt(
            (refv.x - v2.x) ** 2 + (refv.y - v2.y) ** 2
        )  # self.bond_length #(v2.y-refv.y)

        # control
        ang1 = geometry.clockwise_angle_from_east(
            v1.x - refv.x + gx1, v1.y - refv.y + gy1
        )
        ang2 = geometry.clockwise_angle_from_east(
            v2.x - refv.x + gx2, v2.y - refv.y + gy2
        )
        ang = ang1 - ang2
        sign = geometry.on_which_side_is_point(
            (refv.x, refv.y, v1.x, v1.y), (v2.x, v2.y)
...
if __name__ == "__main__":
    ...
    print("converged:", ok)
    print(
        "%d iterations, RMS grad %f, max grad %f" % (opt.i, opt.rms_grad, opt.max_grad)
    )
