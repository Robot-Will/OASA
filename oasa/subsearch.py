...
class substructure_search_manager:
    ...
    def fill_data(self):
        ...
        for struct in subsearch_data.structures:
            ...
            sub = substructure(
                name, compound_type, smiles=smiles_string, atoms_to_ignore=to_ignore
            )
        ...
        while to_delete:
            ...
            for i, hit1 in enumerate(hits):
                for hit2 in hits[i + 1 :]:
                    if hit1 is not hit2:
                        if set(hit1.get_significant_atoms()) & set(
                            hit2.get_significant_atoms()
                        ):
                            winner = self.which_substructure_is_more_specific(
...
""",
        ...
        print(
            "## automatically generated file - may be overwritten at any time", file=out
        )
...
class substructure:
    ...
    def find_matches(self, mol):
        ret = []
        ms = list(
            mol.select_matching_substructures(
...
class substructure_match:
    ...
    def __str__(self):
        return "<Match of %s with %d atoms (%d significant)>" % (
            self.substructure,
            len(self.atoms_found),
            len(self.get_significant_atoms()),
        )
