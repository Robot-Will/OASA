...
class molecule(graph.graph):
    ...
    def get_disconnected_subgraphs(self):
        ...
        for part in out:
            for st in self.stereochemistry:
                if {ref for ref in st.references if isinstance(ref, atom)} <= set(
                    part.vertices
                ):
    ...
    def add_missing_bond_orders(self, retry=False):
        ...
        while processed:
            ...
            for b in [
                bo for bo in self.edges if min(a.free_valency for a in bo.vertices)
            ]:
    ...
    def mark_aromatic_bonds(self):
        ...
        while solved:
            ...
            for aring in rings:
                els = [
                    self._get_atoms_possible_aromatic_electrons(a, aring) for a in aring
                ]
                if () in els:
                    continue
                ...
                if sum(comb) % 4 == 2:
                    ...
                    while i + 1 < len(aring):
                        if comb[i] + comb[i + 1] == 2:
                            ...
                            bs1 = [
                                bo
                                for bo in aring[i].get_neighbor_edges()
                                if bo.order == 2 and bo.aromatic and bo != b
                            ]
                            bs2 = [
                                bo
                                for bo in aring[i + 1].get_neighbor_edges()
                                if bo.order == 2 and bo.aromatic and bo != b
                            ]
    ...
    def localize_fake_aromatic_bonds(self):
        ...
        while to_go:
            ...
            for bo in to_go:
                ...
                if (
                    not bs1
                    or len([e for e in bs1 if e.order == 1]) > 0
                    and len([e for e in bs1 if e.order == 2]) == 0
                    or not bs2
                    or len([e for e in bs2 if e.order == 1]) > 0
                    and len([e for e in bs2 if e.order == 2]) == 0
                ):
                    b = bo
                    break
        ...
        for e1 in ends:
            for e2 in ends:
                if e1 != e2:
                    ...
                    paths.append(
                        path
                    )  # could be optimized by not storing all the paths (once we know how to weight the chains)
    ...
    def select_matching_substructures(
        self, other, implicit_freesites=False, auto_cleanup=True
    ):
        ...
        for v in other.vertices:
            if (
                (isinstance(v, atom) and v.symbol == "H")
                or (isinstance(v, atom) and v.explicit_hydrogens > 0)
                or (
            ...
            if vsset not in yielded:
                if self._freesites_match(other, thread):
                    yield [
                        v
                        for v in vs
                        if not "implicit_hydrogen" in list(v.properties_.keys())
                    ]
            yielded.add(vsset)
    ...
    def clean_after_search(self, other):
        ...
        hs = [
            v
            for v in self.vertices
            if "implicit_hydrogen" in list(v.properties_.keys())
        ]
        for v in hs:
        ...
        for v in other.vertices:
            if "implicit_hydrogen" not in v.properties_:
                v.explicit_hydrogens = len(
                    [h for h in v.neighbors if "implicit_hydrogen" in h.properties_]
                )
        hs = [
            v
            for v in other.vertices
            ...
            for e, n in v.get_neighbor_edge_pairs():
                if thread not in n.properties_["subsearch"]:
                    ...
                    for me, mn in mirror.get_neighbor_edge_pairs():
                        if (
                            thread not in mn.properties_["subsearch"]
                            and mn.matches(n)
                            and me.matches(e)
                            and thread not in e.properties_["subsearch"]
                        ):
                            candidates.add((mn, me, e))

                    if candidates:
                        if len(candidates) > 1:
                            new_threads = self._spawn_thread(
                                other, thread, len(candidates) - 1
                            )
                        ...
                        [
                            x for x in self._mark_matching_threads(n, other)
                        ]  # just make the generator run
                ...
                # for proper handling of rings we have to check also the ones that are in this thread already
                    ...
                    me = self.get_edge_between(
                        mirror, n.properties_["subsearch"][thread]
                    )
    ...
    def _spawn_thread(self, other, thread, number):
        ...
        max_thread = max(
            max(v.properties_["subsearch"].keys())
            for v in other.vertices
            if v.properties_["subsearch"]
        )
    ...
    def _freesites_match(self, other, thread):
        for v in other.vertices:
            mirror = v.properties_["subsearch"][thread]
            unmatched_ns = [
                n for n in mirror.neighbors if thread not in n.properties_["subsearch"]
            ]  # and not n.symbol == 'H']
    ...
    def contains_substructure(self, other, implicit_freesites=True):
        found = False
        for i in self.select_matching_substructures(
            other, implicit_freesites=implicit_freesites, auto_cleanup=False
        ):
            found = True
            break
        self.clean_after_search(other)
    ...
    def mark_morgan(self):
        ...
        for v in self.vertices:
            v.properties_[
                "new_morgan"
            ] = v.degree  # sum( [n.degree for n in v.neighbors])
