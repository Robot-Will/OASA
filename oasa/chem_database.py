...
try:
    ...
    raise Exception(
        "The required pysqlite module could not be loaded. More info here: '%s'" % e
    )

from oasa import inchi


class Config:
    database_file = os.path.abspath(
        os.path.join(os.path.dirname(__file__), "structures.db")
    )
...
def fill_database(infilename, name_cutoff=26, atom_count_cutoff=100):
    ...
    for line in f:
        ...
        if (
            len(name) <= name_cutoff
            and len([x for x in smiles if x.isupper()]) <= atom_count_cutoff
            and _allow_molecule(name, smiles)
        ):
            c.execute(
...
def get_compounds_from_database(database_file=None, **kw):
    ...
    for fname in (database_file, Config.database_file):
        ...
        raise oasa_exceptions.oasa_error(
            "Structure database not found. Try running 'python structure_database.py structures.txt.gz' in oasa directory to create the database file from default source."
        )
    ...
    if search:
        sql = "SELECT structures.* FROM {} WHERE {}".format(
            tables, " AND ".join(search)
        )
    else:
    ...
    try:
        ...
        raise oasa_exceptions.oasa_error(
            "Error reading from structure database: '%s'" % e
        )
    ret = []
    ...
    if options.command == "test":
        ...
        if not options.command == "synonyms":
            if len(args) >= 1:
                ...
                print(
                    "You must supply a valid filename of a file containing structures to be read.",
                    file=sys.stderr,
                )
                sys.exit()
            if options.command == "rebuild":
                if os.path.exists(Config.database_file):
                    try:
                        ...
                        print(
                            "Old database file could not be removed. Reason: %s" % e,
                            file=sys.stderr,
                        )
