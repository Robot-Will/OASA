...
class linear_formula:
    def __init__(self, text="", start_valency=0, end_valency=0, mol=None):
        ...
        if text:
            self.parse_text(
                text, start_valency=start_valency, end_valency=end_valency, mol=mol
            )
        ...
        if mol:
            ...
            for v in mol.vertices:
                if v.free_valency:
                    ...
                    if not (
                        v == self.get_last_free_atom(mol)
                        and v.free_valency == end_valency
                    ):
                        return None
                        ...
                        if last_atom:
                            ...
                            if max_val <= 0 and last_atom.free_valency <= 0:
                                if last_atom.raise_valency():
                                    max_val = min(
                                        last_atom.free_valency, a.free_valency, 3
                                    )
                            b = mol.create_edge()
                            b.order = max_val
                ...
                if chunk:
                    ...
                    for j in range(count):
                        if chunk[0] == "!":
                            ...
                            m = self.parse_form(
                                chunk, start_valency=val, mol=mol.create_graph()
                            )
                            smile = False
                        if not m:
                            return None
