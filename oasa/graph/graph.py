...
TODO: Evaluate replacement with networkx.
"""
import copy
import operator
import warnings
...
from oasa.graph.edge import edge
from oasa.graph.vertex import vertex
...
class graph:
    ...
    def add_vertex(self, v=None):
        ...
        if v not in self.vertices:
            ...
            warnings.warn(
                "Added vertex is already present in graph %s" % str(v), UserWarning, 2
            )
            return None
        self._flush_cache()
        return v
    ...
    def add_edge(self, v1, v2, e=None):
        ...
        if i1 == None or i2 == None:
            warnings.warn(
                "Adding edge to a vertex not present in graph failed (of course)",
                UserWarning,
                3,
            )
            return None
    ...
    def is_tree(self):
        if self.is_connected():
            ...
            return len(self.vertices) - len(
                list(self.get_connected_components())
            ) == len(self.edges)
    ...
    def get_connected_components(self):
        ...
        while not_processed:
            recent = (
                set(reduce(operator.add, [a.get_neighbors() for a in recent], []))
                & not_processed
            )
            if not recent:
                if comp:
    ...
    def get_smallest_independent_cycles(self):
        ...
        return list(
            map(
                self.edge_subgraph_to_vertex_subgraph,
    ...
    def get_all_cycles_e_old(self):
        ...
        while to_go:
            ...
            to_go -= {
                ...
                for ver in reduce(
                    operator.or_,
                    list(map(self.edge_subgraph_to_vertex_subgraph, cycles)),
                    set(),
                )
                if ver.degree == 2
            }
    ...
    def _get_cycles_for_vertex(self, v, to_reach=None, processed=None):
        ...
        for e, neigh in v.get_neighbor_edge_pairs():
            if neigh not in processed:
                p = {v} | processed
                cycles = self._get_cycles_for_vertex(
                    neigh, to_reach=to_reach, processed=p
                )
                if cycles:
                    for cycle in cycles:
                        cycle.add(e)
    ...
    def get_smallest_independent_cycles_e(self):
        ...
        if ncycles < 0:
            warnings.warn(
                "The number of edges is smaller than number of vertices-1, the molecule must be disconnected, which means there is something wrong with it.",
                UserWarning,
                3,
            )
            ncycles = 0
        ...
        while vs and len(cycles) < ncycles:
            ...
            if removed_e:
                ...
                for cycle in new_cycles:
                    ...
                    to_go = {
                        v
                        for v in self.edge_subgraph_to_vertex_subgraph(cycle)
                        if v.degree == 2
                    }
                    while to_go:
                        now = {to_go.pop()}
                        ...
                        while now:
                            now = reduce(
                                operator.or_,
                                [
                                    {n for n in v.neighbors if n.degree == 2}
                                    for v in now
                                ],
                            )
                            now &= to_go
        ...
        if len(cycles) < ncycles:
            warnings.warn(
                "The number of cycles found (%d) is smaller than the theoretical value %d (|E|-|V|+1)"
                % (len(cycles), ncycles),
                UserWarning,
                2,
            )
            ...
            warnings.warn(
                "The number of independent cycles found (%d) is larger than the theoretical value %d (|E|-|V|+1), but I cannot improve it."
                % (len(cycles), ncycles),
                UserWarning,
                2,
            )
    ...
    def _get_smallest_cycle_for_vertex(
        self, v, to_reach=None, came_from=None, went_through=None
    ):
        ...
        for e, neigh in v.get_neighbor_edge_pairs():
            if (not went_through or neigh not in went_through) and not e == came_from:
                gens.append(
                    self._get_smallest_cycle_for_vertex(
                        neigh, to_reach=to_reach, came_from=e, went_through=w
                    )
                )
        while 1:
    ...
    def _get_smallest_cycles_for_vertex(
        self, v, to_reach=None, came_from=None, went_through=None
    ):
        ...
        for e, neigh in v.get_neighbor_edge_pairs():
            ...
            if (not went_through or neigh not in went_through) and not e == came_from:
                gens.append(
                    self._get_smallest_cycles_for_vertex(
                        neigh, to_reach=to_reach, came_from=e, went_through=w
                    )
                )
        ...
        while 1:
            ...
            if counter > 10000:
                raise Exception(
                    "This exception is triggered when ring-searching code does not find anything in 10000 steps. This means that there is with high probability something wrong with the code and it would be futile to continue in an endless search. Please let me know at beda@zirael.org where this happened"
                )
    ...
    def get_all_cycles_old(self):
        ...
        return list(
            map(self.edge_subgraph_to_vertex_subgraph, self.get_all_cycles_e_old())
        )
    ...
    def _p_graph_remove(v, pgraph):
        ...
        for i, (ne1, nv1) in enumerate(neighbor_edge_vertex_pairs):
            for ne2, nv2 in neighbor_edge_vertex_pairs[i + 1 :]:
                if (nv1 is nv2 and (ne1.path_ & ne2.path_ == {v, nv2})) or (
                    ne1.path_ & ne2.path_ == {v}
                ):
    ...
    def get_path_between_edges(self, e1, e2):
        ...
        if not "dist" in e2.properties_:
            ...
            for i in range(e2.properties_["dist"] - 1, -1, -1):
                _e = [
                    ee for ee in _e.get_neighbor_edges() if ee.properties_["dist"] == i
                ][0]
                path.append(_e)
            return path
    ...
    def _get_width_from_vertex(self, v):
        ...
        while to_mark:
            ...
            marked = to_mark
            to_mark_next = set(
                reduce(operator.add, [i.get_neighbors() for i in to_mark], [])
            )
    ...
    def defines_connected_subgraph_e(self, edges):
        sub = self.get_new_induced_subgraph(
            self.edge_subgraph_to_vertex_subgraph(edges), edges
        )
        return sub.is_connected()

    def defines_connected_subgraph_v(self, vertices):
        sub = self.get_new_induced_subgraph(
            vertices, self.vertex_subgraph_to_edge_subgraph(vertices)
        )
        return sub.is_connected()
    ...
    def find_path_between(self, start, end, dont_go_through=[]):
        ...
        for i in range(d, 0, -1):
            vs = [
                v
                for e, v in rend.get_neighbor_edge_pairs()
                if v.properties_["d"] == i - 1
                and (v not in dont_go_through or e not in dont_go_through)
            ]
            if not vs:
                return None
            v = vs[0]
...
def is_ring_end_vertex(v):
    ...
    for x in v.get_neighbors():
        if v.properties_["d"] == x.properties_["d"]:
            if len(v.get_neighbors_with_distance(v.properties_["d"] - 1)) and len(
                x.get_neighbors_with_distance(v.properties_["d"] - 1)
            ):
                ed = {x, v}
...
def filter_off_dependent_cycles(cycles):
    ...
    while level < len(cs):
        to_del = set()
        for c in cs:
            combs = gen_variations(
                [x for x in cs if x != c and x not in to_del and x & c], level
            )
            for combl in combs:
                comb = set(combl)
                if (
                    not comb & to_del
                    and (c <= reduce(operator.or_, comb))
                    and reduce(operator.and_, comb)
                ):
                    to_del.add(c)
                    break
