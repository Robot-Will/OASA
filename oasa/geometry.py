...
def point_on_circle(center_x, center_y, radius, direction=(), resolution=15):
    ...
    dx, dy = direction
    if resolution:
        angle = round(atan2(dy, dx) / (pi * resolution / 180.0)) * (
            pi * resolution / 180.0
        )
    else:
        angle = atan2(dy, dx)
...
def angle_between_lines(l1, l2):
    ...
    a = clockwise_angle_from_east(
        l1[2] - l1[0], l1[3] - l1[1]
    ) - clockwise_angle_from_east(l2[2] - l2[0], l2[3] - l2[1])
    if a > pi:
        return a - 2 * pi
    elif a < -pi:
        return a + 2 * pi
...
def tkspline_to_cubic_bezier(points):  # points = ((x1,y1),(x2,y2),...)
    ...
    for qbez in qbeziers:
        ...
        ccurvexa, ccurveya = startx + 2 / 3 * (qcurvex - startx), starty + 2 / 3 * (
            qcurvey - starty
        )
        ccurvexb, ccurveyb = ccurvexa + 1 / 3 * (endx - startx), ccurveya + 1 / 3 * (
            endy - starty
        )
        cbeziers.append(
            (startx, starty, ccurvexa, ccurveya, ccurvexb, ccurveyb, endx, endy)
        )
...
def intersection_of_two_lines(
    x1, y1, x2, y2, x3, y3, x4, y4, parallel_detection_threshold=3
):
    """lines 1-2 and 3-4
...
def find_parallel_polyline(coords, d):
    ...
    for x, y in coords:
        if i == 0:
            ...
            xx, yy, parallel, online = intersection_of_two_lines(
                xa1, ya1, xa2, ya2, xb1, yb1, xb2, yb2, parallel_detection_threshold=1
            )
            if not parallel:
                if online:
                    ...
                    if point_distance(xa, ya, xa2, ya2) < point_distance(
                        xa, ya, xx, yy
                    ):
...
def is_point_beween_points_of_line(line, point):
    ...
    x1, y1, x2, y2 = line
    x, y = point
    if point_distance(x1, y1, x, y) + point_distance(
        x2, y2, x, y
    ) > 1.02 * point_distance(x1, y1, x2, y2):
        return False
    return True
