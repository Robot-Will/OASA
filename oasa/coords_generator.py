...
class coords_generator:
    ...
    def calculate_coords(self, mol, bond_length=0, force=0):
        ...
        if len(atms) == len(mol.vertices) and not force:
            ...
            sub = mol.get_new_induced_subgraph(
                atms, mol.vertex_subgraph_to_edge_subgraph(atms)
            )
        ...
        while processed:
            ...
            for v in processed:
                ...
                if not ring:
                    ...
                    if (
                        len(processed) > 1
                        and mol.defines_connected_subgraph_v(processed)
                        and set(processed) <= ring
                    ):
                        ...
                        ring.remove(v)
                        d = [
                            a for a in v.get_neighbors() if a.x != None and a.y != None
                        ][
                            0
                        ]  # should always work
    ...
    def process_atom_neigbors(self, v):
        def get_angle_at_side(v, d, d2, relation, attach_angle):
            ...
            if relation * side == geometry.on_which_side_is_point(
                (d.x, d.y, v.x, v.y), (x, y)
            ):
                return attach_angle
            else:
        ...
        if len(done) == 1 and (
            len(to_go) == 1
            or len(to_go) == 2
            and [1 for _t in to_go if _t in self.stereo]
        ):
            ...
            if t in self.stereo:
                ss = [
                    st
                    for st in self.stereo[t]
                    if not None in st.get_other_end(t).coords[:2]
                ]
                if ss:
            ...
            # branched chain
            angles = [
                geometry.clockwise_angle_from_east(at.x - v.x, at.y - v.y)
                for at in done
            ]
            ...
            gcoords = gen_coords_from_stream(
                gen_angle_stream(angle, start_from=angles[i + 1] + angle),
    ...
    def _process_simply_anelated_ring(self, ring, base):
        ...
        if len(inter) == 1:
            ...
            if not v1 in ring[0].get_neighbors():
                v1, v2 = v2, v1
            side = sum(
                geometry.on_which_side_is_point((v1.x, v1.y, v2.x, v2.y), (v.x, v.y))
                for v in base
            )
            if not side:
    ...
    def _process_multi_anelated_ring(self, ring, angle_shift=0):
        ...
        if not sorted_back:
            ...
            ca = deg_to_rad(
                180
                - (overall_angle - blocked_angle - len(to_go) * da + angle_shift) / 2
            )  # connection angle
            side = sum(
                geometry.on_which_side_is_point((v1.x, v1.y, v2.x, v2.y), (v.x, v.y))
                for v in back
            ...
            if (
                geometry.on_which_side_is_point((v1.x, v1.y, v3.x, v3.y), (v2.x, v2.y))
                < 0
            ):
                da = -da
...
def gen_ring_coords(size, side_length=1):
    coords = gen_coords_from_deg_stream(
        gen_angle_stream(180 - 180.0 * (size - 2) / size), length=side_length
    )
    x, y = 0, 0
    for i in range(size):
        dx, dy = next(coords)
