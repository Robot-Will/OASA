...
def cairo_out_test2():
    ...
    mol.remove_unimportant_hydrogens()
    c = cairo_out.cairo_out(
        color_bonds=True, color_atoms=True
    )  # , background_color=(0.4,1,0.5,0.8))
    c.show_hydrogens_on_hetero = True
    c.font_size = 20
...
def cairo_out_test_wedge():
    ...
    mol.remove_unimportant_hydrogens()
    c = cairo_out.cairo_out(
        scaling=1.0, antialias_text=True, color_bonds=True, color_atoms=True
    )
    t = time.time()
...
def test_new_ring_perception():
    ...
    mol = smiles.text_to_mol(
        "C2369C478C56C179C34C1258", calc_coords=False, localize_aromatic_bonds=False
    )
    t = time()
