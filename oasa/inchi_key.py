...
def compute_inchi_check(key):
    ...
    if not m:
        raise Exception(
            "The submitted string does not have the format of an base of InChIKey ('^[A-Z]{14}-[A-Z]{9}$')"
        )
...
def key_from_inchi_old(inp):
    ...
    if not version.isdigit():
        if len(parts) == 1:
            ...
            raise Exception(
                f"Invalid data in version part of InChI - '{version}' in '{inp}'"
            )
    ...
    base = (
        ...
        + "-"
        + minor_digest(minor)
        + flag_old(int(version), parts_minor)
    )
...
def check_inchi_key(key):
    ...
    if not m:
        raise Exception(
            "The submitted string does not have the format of an InChIKey ('[A-Z]{14}-[A-Z]{10}')"
        )
    base = m.group(1) + "-" + m.group(2)
...
def key_from_inchi(inp):
    ...
    if not m:
        raise Exception(
            f"Invalid data in version part of InChI - '{parts[0]}' in '{inp}'"
        )
    else:
    ...
    base = (
        ...
        + minor_digest(minor)
        + standard
        + string.ascii_uppercase[int(version) - 1]
    )
