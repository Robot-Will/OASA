...
class formula_dict(dict):
        ...
        if type(form) in (bytes, str):
            ...
            for key, val in form.items():
                if key in periodic_table and type(val) == int:
                    ...
                    raise ValueError(
                        f"some of the dictionary entries are not valid for formula_dict ({str( key)} => {str( val)})"
                    )
    ...
    def to_tuple(self):
        return tuple(
            reduce(operator.add, [[k, self[k]] for k in self.sorted_keys()], [])
        )
